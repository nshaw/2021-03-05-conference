...
import { apiTracksGet, apiTracksDelete } from 'api/tracks';
import { reducer, initialState } from 'state/track.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/track.types';
...
class TrackTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated) {
      try {
        ...
        const { tracks, headers } = await apiTracksGet(serviceUrl, requestParameters);
        const trackCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...tracks] : tracks,
            count: trackCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiTracksDelete(serviceUrl, item.id);
        onDelete(item);
    ...
    const Actions = ({ item }) =>
      onDelete ? (
        <ConfirmationDialogTrigger
          ...
          dialog={{
            title: t('entities.track.deleteDialog.title'),
            description: t('entities.track.deleteDialog.description'),
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <TrackTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
TrackTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(TrackTableContainer))
  )
);
